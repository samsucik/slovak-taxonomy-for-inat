...
const relevantSlovakTaxonRankNames = ["Rad", "Podrad", "Nadčeľaď", "Čeľaď", "Podčeľaď", "Rod", "Druh"];
...
function taxonMatchesScientificName(taxonHeaderText, scientificName) {
  ...
  if (relevantSlovakTaxonRankNames.some(taxonRank => taxonHeaderText.toLowerCase().replace(taxonRank.toLowerCase(), "").trim() == scientificName.toLowerCase())) {
    return true;
  }
...
async function closeTabIfApplicable() {
  ...
  if (dataRaw) {
    ...
    if (document.querySelector('#TaxonHeader .sciname')) {
      ...
      if (taxonMatchesScientificName(document.querySelector('#TaxonHeader .sciname').innerText, data.sci)) {
